...
The root command and all subcommands accept the following flags to set the log level for `precious`:
...
The root command and all subcommands accept the following flags to set the output style for
`precious`:
...
When tidying files, `precious` tries to report whether files have been changed by a tidying command.
It does this by calculating a checksum for a file before it's tidied, and then recalculating that
checksum after the tidying command runs.
...
files, this could be a huge number of files, so `precious` simply skips the checksumming. That means
that it can't determine whether the file was actually changed by the tidying command, which is why
it says "maybe changed".
