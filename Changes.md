...
- Made all info-level logging truncate the command being run when the full command contains many
  ...
  GH #80.
- Added a new `--no-color` flag that suppresses all ANSI colors in the output. Requested by
  @danielcompton (Daniel Compton). GH #82.
...
- All config keys have been changed to use dashes instead of underscores, so for example `path_args`
  is now `path-args` and `ok_exit_codes` is now `ok-exit-codes`. However, the names with underscores
...
- If precious was run with `--staged` and a file that was staged had been deleted from the
  ...
  it will simply ignore such a deleted file, though it will log a debug-level message saying that
  the file is being ignored. GH #63.
