...
It's odd to combine `working-dir = "dir"` with passing absolute files, but it
will work.
...
Since the root directory is not included in the `sub-roots`, the command is
